 ...
 */
 ...
 function render(solucionLocal,
                 ...
                 separador=" " ) {
     // Letra y guiones
     const finalHTML = solucionLocal.split("").map( (letra) => {
         return letrasUsuario.includes(letra) ? letra : vacio;
     });
     // Numero de intentos
     const temp = document.querySelector('#numero-intentos')
     temp.textContent = numeroIntentosLocal;
 ...
 */
 ...
 nuevaLetra.addEventListener('keyup', function (event) {
    ...
    if (event.key === "Enter") {
        anyadirLetra();
        render(solucion, letrasIntroducidas, visualJugador, intentos, numeroIntentos);
    }
 })
